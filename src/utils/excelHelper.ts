...
export function exportPoliciesToExcel(policies: PolicyRenewal[], fileName = 'Cartera_Renovacion_GXP.xlsx') {
  const data = policies.map((p, idx) => ({
    ...
    'Tipo Renovación': p.tipoRenovacion === 'CAMBIO_PLAN' ? 'Cambio Automático de Plan' : 'Ajuste de Tasa',
    'Plan a Renovar': p.planRenovacion || p.descPlanProd || p.cobertura,
    '% Incremento / Ajuste': p.tipoRenovacion === 'CAMBIO_PLAN' ? 'No aplica' : `${p.porcentajeIncremento}%`,
